/**
 * The AI Creator's Marketing Haus — Look Lock
...
(function () {
  ...
  var STORAGE_KEY = "marketingHausLookLocks";
  var MAX_LOOKS = 5;

  // Universal aesthetic vocabulary every generator's `aesthetic`-flagged
  // fields map onto — see marketing-haus-generators.js's applyActiveLookToGenerator.
  ...
  function createLook(name) {
    ...
    var look = { id: "mhlook-" + Date.now() + "-" + Math.floor(Math.random() * 10000), name: (name || "").trim() || "Untitled Look", createdAt: Date.now(), fields: buildLookFields() };
  ...
  // (per-generator apply/capture is wired from marketing-haus-generators.js).
  // ---------------------------------------------------------------------
  ...
  function renderLookFields(look) {
    ...
    LOOK_FIELD_NAMES.forEach(function (name) {
      fieldsWrap.appendChild(ui.renderFreeTextField(
        ...
        function (changes) { updateLookField(look.id, name, changes); MarketingHaus.ui.renderApp(); }
      ));
    });
    return fieldsWrap;
  }
  ...
  function renderLookCard(look, isActive) {
    ...
    if (renamingLookId === look.id) {
      var titleInput = ui.el("input", { type: "text", class: "mh-saved__item-title-input", value: look.name });
      var confirmRename = function () { renameLook(look.id, titleInput.value.trim() || look.name); renamingLookId = null; MarketingHaus.ui.renderApp(); };
      titleInput.addEventListener("keydown", function (e) { if (e.key === "Enter") confirmRename(); if (e.key === "Escape") { renamingLookId = null; MarketingHaus.ui.renderApp(); } });
      ...
      renameBtn.addEventListener("click", function () { renamingLookId = look.id; MarketingHaus.ui.renderApp(); });
      titleRow = ui.el("div", { class: "mh-saved__item-title-row" }, [
        ui.el("p", { class: "mh-saved__item-title", text: look.name + (isActive ? " (active)" : "") }),
        renameBtn,
      ]);
    }
    ...
    activeBtn.addEventListener("click", function () { setActiveLook(isActive ? null : look.id); MarketingHaus.ui.renderApp(); });

    var expandBtn = ui.el("button", { type: "button", class: "mh-btn mh-btn--small", text: isExpanded ? "Hide Fields" : "Edit Fields" });
    expandBtn.addEventListener("click", function () { expandedLookId = isExpanded ? null : look.id; MarketingHaus.ui.renderApp(); });

    var deleteBtn = ui.el("button", { type: "button", class: "mh-btn mh-btn--delete mh-btn--small", text: "Delete" });
    deleteBtn.addEventListener("click", function () { deleteLook(look.id); MarketingHaus.ui.renderApp(); });
  ...
  function renderSection(root) {
    ...
    if (!looks.length) {
      list.appendChild(ui.el("p", { class: "mh-saved__empty", text: "No Looks yet — build a generator result you love, then click \"Lock This Look\" below its fields, or create one here." }));
    } else {
      // Collapsed by default to just the active Look (or the first one if none is active).
    ...
    var createRow;
    if (!isFull()) {
      var nameInput = ui.el("input", { type: "text", class: "mh-field__custom", placeholder: "New Look name, e.g. \"Dark Academia Journal\"" });
      var createBtn = ui.el("button", { type: "button", class: "mh-btn mh-btn--add mh-btn--small", text: "+ Create Look" });
      ...
      createRow = ui.el("p", { class: "mh-field-group__subtitle", text: "You have " + MAX_LOOKS + "/" + MAX_LOOKS + " Looks — delete one to create another." });
    }

    var headerChildren = [ui.el("h3", { class: "mh-saved__title" }, [ui.icon("palette"), ui.el("span", { text: "Look Lock (" + looks.length + "/" + MAX_LOOKS + ")" })])];
    if (looks.length > 1) {
      ...
      toggleBtn.addEventListener("click", function () { lookLockExpanded = !lookLockExpanded; MarketingHaus.ui.renderApp(); });
      headerChildren.push(toggleBtn);
    }
    root.appendChild(ui.el("div", { class: "mh-saved" }, [
      ui.el("div", { class: "mh-faq__header" }, headerChildren),
      ui.el("p", { class: "mh-field-group__subtitle", text: "Lock one generator's art style, palette, mood, and texture, then every other generator's matching fields pick it up automatically — the fix for a second page not matching the first." }),
      list,
      createRow,
    ]));
  }

  MarketingHaus.lookLock = {
    MAX_LOOKS: MAX_LOOKS,
