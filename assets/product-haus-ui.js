...
(function () {
  ...
  var STEPS = [
    { icon: "step1", title: "Choose", subtitle: "Pick your category" },
  ...
  function renderStepper(root) {
    var row = el("div", { class: "pdh-stepper" });
    STEPS.forEach(function (step, index) {
      row.appendChild(
        el("div", { class: "pdh-stepper__step" }, [
          icon(step.icon, "pdh-stepper__icon"),
          el("div", { class: "pdh-stepper__text" }, [
            el("span", { class: "pdh-stepper__title", text: (index + 1) + " " + step.title }),
            el("span", { class: "pdh-stepper__subtitle", text: step.subtitle }),
          ]),
        ])
      );
      if (index < STEPS.length - 1) row.appendChild(el("div", { class: "pdh-stepper__connector" }));
    });
    root.appendChild(row);
  }

  var TIPS = [
    "Fill in the Product Information once — name, type, audience — and every category builds on that same foundation.",
    "Pick ONE category per prompt; a listing and an ad want different framing.",
    "\"Or type your own...\" always overrides the dropdown — use it for details specific to your product.",
    "Set Target Platform before you copy so the output matches where it's going.",
    "Stuck? Hit Randomize, then swap the 1-2 things you don't love instead of starting over.",
    "Save combinations to Your Vault so you can revisit and tweak instead of rebuilding from memory.",
    "Set up a Brand Kit once and every category inherits your voice and look automatically.",
    "Be specific — a real benefit or a real objection gives sharper output than a generic description.",
  ];

  var tipsExpanded = false;

  function renderTipsPanel(root) {
    var toggleBtn = el("button", { type: "button", class: "pdh-tips__toggle" }, [
    ...
    var children = [
      el("div", { class: "pdh-tips__header" }, [
        el("h3", { class: "pdh-tips__title" }, [icon("bulb"), el("span", { text: "Tips for Better Prompts" })]),
        toggleBtn,
      ]),
    ];

    if (tipsExpanded) {
      var list = el("ul", { class: "pdh-tips__list" });
    ...
    root.appendChild(el("div", { class: "pdh-tips" }, children));
  }
  ...
  function renderAppContent(root, focusRestore, scrollX, scrollY, previewHeights) {
    ...
    renderStepper(shell);
    shell.appendChild(el("hr", { class: "pdh-section-divider" }));
