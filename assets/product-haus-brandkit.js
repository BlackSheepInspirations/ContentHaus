...
(function () {
  ...
  var STORAGE_KEY = "productHausBrandKits";
  ...
  var store = ProductHaus.util.createStore(readPersisted());
  ...
  function createKit(name) {
    ...
    var kit = { id: "pdhbk-" + Date.now() + "-" + Math.floor(Math.random() * 10000), name: (name || "").trim() || "Untitled Brand Kit", createdAt: Date.now(), fields: buildKitFields() };
  ...
  function resolved(field) {
    return ProductHaus.engine.resolveFieldValue(field);
  }
  ...
  function renderKitFields(kit) {
    ...
    var fieldsWrap = ui.el("div", { class: "pdh-field-group__fields" });
    ...
    fieldsWrap.appendChild(ui.renderColorPickerList({
      ...
      onUpdate: function (i, hex) { var next = kit.fields.colors.slice(); next[i] = hex; updateKitColors(kit.id, next); ProductHaus.ui.renderApp(); },
      onAdd: function () { updateKitColors(kit.id, kit.fields.colors.concat(["#6B6860"])); ProductHaus.ui.renderApp(); },
      onRemove: function (i) { updateKitColors(kit.id, kit.fields.colors.filter(function (_, idx) { return idx !== i; })); ProductHaus.ui.renderApp(); },
    }));

    var typographyFields = ui.el("div", { class: "pdh-field-group__fields" }, [
      ui.renderFontPreviewField({ label: "Heading Font", field: kit.fields.headingFont }, function (changes) { updateKitField(kit.id, "headingFont", changes); ProductHaus.ui.renderApp(); }),
      ui.renderFontPreviewField({ label: "Body Font", field: kit.fields.bodyFont }, function (changes) { updateKitField(kit.id, "bodyFont", changes); ProductHaus.ui.renderApp(); }),
    ]);
    ...
    fieldsWrap.appendChild(ui.renderTextSlotList({
      ...
      onAdd: function () { updateKitValues(kit.id, kit.fields.coreValues.concat([""])); ProductHaus.ui.renderApp(); },
      onRemove: function (i) { updateKitValues(kit.id, kit.fields.coreValues.filter(function (_, idx) { return idx !== i; })); ProductHaus.ui.renderApp(); },
    }));

    fieldsWrap.appendChild(ui.renderFreeTextField({ label: "What the Brand is NOT (optional)", field: kit.fields.notThis, placeholder: "e.g. \"corporate, cold, overly polished\"" }, function (changes) { updateKitField(kit.id, "notThis", changes); ProductHaus.ui.renderApp(); }));
  ...
  function renderKitCard(kit, isActive) {
    ...
    if (!isSynced && renamingKitId === kit.id) {
      var titleInput = ui.el("input", { type: "text", class: "pdh-saved__item-title-input", value: kit.name });
      var confirmRename = function () { renameKit(kit.id, titleInput.value.trim() || kit.name); renamingKitId = null; ProductHaus.ui.renderApp(); };
      titleInput.addEventListener("keydown", function (e) { if (e.key === "Enter") confirmRename(); if (e.key === "Escape") { renamingKitId = null; ProductHaus.ui.renderApp(); } });
      titleInput.addEventListener("blur", confirmRename);
      titleRow = ui.el("div", { class: "pdh-saved__item-title-row" }, [titleInput]);
    } else {
      var titleChildren = [ui.el("p", { class: "pdh-saved__item-title", text: kit.name + (isActive ? " (active)" : "") })];
      if (!isSynced) {
        var renameBtn = ui.el("button", { type: "button", class: "pdh-saved__rename-btn", title: "Rename" }, [ui.icon("edit")]);
        renameBtn.addEventListener("click", function () { renamingKitId = kit.id; ProductHaus.ui.renderApp(); });
        titleChildren.push(renameBtn);
      }
      titleRow = ui.el("div", { class: "pdh-saved__item-title-row" }, titleChildren);
    }

    var activeBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--small " + (isActive ? "pdh-btn--reset" : "pdh-btn--copy"), text: isActive ? "Turn Off" : "Set Active" });
    activeBtn.addEventListener("click", function () { setActiveKit(isActive ? null : kit.id); ProductHaus.ui.renderApp(); });

    var actionBtns = [activeBtn];
    if (isSynced) {
      actionBtns.push(ui.el("span", { class: "pdh-brandkit__synced-badge", text: "From your Brand DNA Blueprint™" }));
    } else {
      var expandBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--small", text: isExpanded ? "Hide Fields" : "Edit Fields" });
      expandBtn.addEventListener("click", function () { expandedKitId = isExpanded ? null : kit.id; ProductHaus.ui.renderApp(); });
      var deleteBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--delete pdh-btn--small", text: "Delete" });
      deleteBtn.addEventListener("click", function () { deleteKit(kit.id); ProductHaus.ui.renderApp(); });
      actionBtns.push(expandBtn, deleteBtn);
    }

    var children = [titleRow, ui.el("div", { class: "pdh-saved__item-actions" }, actionBtns)];
  ...
  function renderSection(root) {
    ...
    var list = ui.el("div", { class: "pdh-saved__list" });
    if (!kits.length) {
    ...
    if (!isFull()) {
      var nameInput = ui.el("input", { type: "text", class: "pdh-field__custom", placeholder: "New kit name, e.g. \"Client: Wildroot\"" });
      var createBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--add pdh-btn--small", text: "+ Create Brand Kit" });
      createBtn.addEventListener("click", function () {
        var result = createKit(nameInput.value);
        if (result.ok) ProductHaus.ui.renderApp();
      });
      createRow = ui.el("div", { class: "pdh-companion__controls" }, [nameInput, createBtn]);
    } else {
