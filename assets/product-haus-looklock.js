...
(function () {
  ...
  var STORAGE_KEY = "productHausLookLocks";
  var MAX_LOOKS = 5;

  // Universal aesthetic vocabulary every generator's `aesthetic`-flagged
  // fields map onto — see product-haus-generators.js's applyActiveLookToGenerator.
  ...
  function createLook(name) {
    ...
    var look = { id: "pdhlook-" + Date.now() + "-" + Math.floor(Math.random() * 10000), name: (name || "").trim() || "Untitled Look", createdAt: Date.now(), fields: buildLookFields() };
  ...
  // (per-generator apply/capture is wired from product-haus-generators.js).
  // ---------------------------------------------------------------------
  ...
  function renderLookFields(look) {
    ...
    LOOK_FIELD_NAMES.forEach(function (name) {
      fieldsWrap.appendChild(ui.renderFreeTextField(
        ...
        function (changes) { updateLookField(look.id, name, changes); ProductHaus.ui.renderApp(); }
      ));
    });
    return fieldsWrap;
  }
  ...
  function renderLookCard(look, isActive) {
    ...
    if (renamingLookId === look.id) {
      var titleInput = ui.el("input", { type: "text", class: "pdh-saved__item-title-input", value: look.name });
      var confirmRename = function () { renameLook(look.id, titleInput.value.trim() || look.name); renamingLookId = null; ProductHaus.ui.renderApp(); };
      titleInput.addEventListener("keydown", function (e) { if (e.key === "Enter") confirmRename(); if (e.key === "Escape") { renamingLookId = null; ProductHaus.ui.renderApp(); } });
      ...
      renameBtn.addEventListener("click", function () { renamingLookId = look.id; ProductHaus.ui.renderApp(); });
      titleRow = ui.el("div", { class: "pdh-saved__item-title-row" }, [
        ui.el("p", { class: "pdh-saved__item-title", text: look.name + (isActive ? " (active)" : "") }),
        renameBtn,
      ]);
    }
    ...
    activeBtn.addEventListener("click", function () { setActiveLook(isActive ? null : look.id); ProductHaus.ui.renderApp(); });

    var expandBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--small", text: isExpanded ? "Hide Fields" : "Edit Fields" });
    expandBtn.addEventListener("click", function () { expandedLookId = isExpanded ? null : look.id; ProductHaus.ui.renderApp(); });

    var deleteBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--delete pdh-btn--small", text: "Delete" });
    deleteBtn.addEventListener("click", function () { deleteLook(look.id); ProductHaus.ui.renderApp(); });

    var children = [titleRow, ui.el("div", { class: "pdh-saved__item-actions" }, [activeBtn, expandBtn, deleteBtn])];
  ...
  function renderSection(root) {
    ...
    var createRow;
    if (!isFull()) {
      var nameInput = ui.el("input", { type: "text", class: "pdh-field__custom", placeholder: "New Look name, e.g. \"Dark Academia Journal\"" });
      var createBtn = ui.el("button", { type: "button", class: "pdh-btn pdh-btn--add pdh-btn--small", text: "+ Create Look" });
