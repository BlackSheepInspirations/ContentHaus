 ...
 * but before graphics-haus-ui.js's own renderApp() ever runs (deferred
 ...
 *
 * This file itself only reaches into GraphicsHaus.ui lazily, inside
 * functions that don't run until a user actually opens the Quick
 * Generators tab — same deferred-call convention graphics-haus-ui.js's
 * own header comment documents — so it can sit anywhere in the script
 * list relative to graphics-haus-ui.js itself.
 *
...
(function () {
  ...
  var isNoneSelection = GraphicsHaus.engine.isNoneSelection;
  var resolveFields = GraphicsHaus.engine.resolveFields;
  ...
  // (artStyle/palette/mood/texture/motifs, see graphics-haus-looklock.js).
  // Everything here is inert if graphics-haus-looklock.js hasn't loaded.
  // ---------------------------------------------------------------------
  ...
  function ensureLookLockApplied(id) {
    if (!GraphicsHaus.lookLock || !hasAestheticFields(getDef(id))) return;
    var store = getStore(id);
    var state = store.getState();
  ...
  function renderLabeledBlocksSection(titleText, blocks) {
    ...
    blocks.forEach(function (v) {
      ...
      wrap.appendChild(ui.el("div", { class: "gh-generator-variation" }, [
        ui.el("div", { class: "gh-generator-variation__header" }, [
  ...
  function renderBundleBlock(id) {
    ...
    saveBtn.addEventListener("click", function () {
      ...
      var result = GraphicsHaus.favorites.save(vaultKey, { text: combined, title: title, snapshot: ui.buildVaultSnapshot(vaultKey) });
      bundleSaveFeedback = result.ok ? "Saved!" : result.reason;
      GraphicsHaus.ui.renderApp();
      setTimeout(function () { bundleSaveFeedback = null; GraphicsHaus.ui.renderApp(); }, 2500);
    });
    var row = ui.el("div", { class: "gh-companion__controls" }, [saveBtn]);
    if (bundleSaveFeedback) row.appendChild(ui.el("span", { style: "color: var(--gh-teal); font-weight: 600; font-size: 13px;", text: bundleSaveFeedback }));
  ...
  function renderPageTypesPicker(id, def, state) {
    ...
    wrap.appendChild(ui.el("legend", { class: "gh-field-group__title" }, [ui.icon("layers"), ui.el("span", { text: def.pageTypesLabel || "Pages" })]));
    var subtitleText = chosen.length + " of " + cap + " selected" + (chosen.length === 0 ? " — leave blank and we'll use a solid default set" : "");
    wrap.appendChild(ui.el("p", { class: "gh-field-group__subtitle", text: subtitleText }));
    var row = ui.el("div", { class: "gh-pill-toggle" });
    def.pageTypes.forEach(function (pt) {
      var isActive = chosen.indexOf(pt.id) !== -1;
      var btn = ui.el("button", { type: "button", class: "gh-pill-toggle__btn" + (isActive ? " is-active" : "") }, [ui.el("span", { text: pt.label })]);
  ...
  function renderSectionsPicker(id, def, state) {
    ...
    var cap = def.sectionsCap || 4;
    var wrap = ui.el("fieldset", { class: "gh-field-group" });
    wrap.appendChild(ui.el("legend", { class: "gh-field-group__title" }, [ui.icon("layers"), ui.el("span", { text: def.sectionsLabel || "Sections" })]));
    var subtitleText = chosen.length + " of " + cap + " selected" + (chosen.length === 0 ? " — leave blank and we'll use a solid default set" : "");
    wrap.appendChild(ui.el("p", { class: "gh-field-group__subtitle", text: subtitleText }));
    (def.sectionGroups || []).forEach(function (group) {
      wrap.appendChild(ui.el("p", { class: "gh-imagery__category-label", text: group.label }));
      var row = ui.el("div", { class: "gh-pill-toggle" });
      group.options.forEach(function (opt) {
        var isActive = chosen.indexOf(opt) !== -1;
        var btn = ui.el("button", { type: "button", class: "gh-pill-toggle__btn" + (isActive ? " is-active" : "") }, [ui.el("span", { text: opt })]);
        btn.disabled = !isActive && chosen.length >= cap;
        btn.addEventListener("click", function () {
          var next = chosen.slice();
  ...
  function renderLockThisLookRow(id, def) {
    var ui = GraphicsHaus.ui;
    var btn = ui.el("button", { type: "button", class: "gh-btn gh-btn--small gh-btn--add", text: "Lock This Look" });
    ...
    btn.addEventListener("click", function () {
      ...
      GraphicsHaus.lookLock.captureIntoActiveOrNewLook(map, def.label);
      GraphicsHaus.ui.renderApp();
    });
    return ui.el("div", { class: "gh-companion__controls" }, [btn]);
  }
  ...
  function renderGeneratorPanel(id) {
    ...
    if (hasAestheticFields(def) && GraphicsHaus.lookLock) wrap.appendChild(renderLockThisLookRow(id, def));
  ...
  GraphicsHaus.generators = {
    ...
    // The Vault/Recent Log snapshot machinery in graphics-haus-ui.js reads
    // and writes state via `GraphicsHaus[mode].getState()/.setState()` —
    // Quick Generators' vault key ("gen:<id>") doesn't map to a top-level
    // GraphicsHaus property the way every other mode's does, so it reads
