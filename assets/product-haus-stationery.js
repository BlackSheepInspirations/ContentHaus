 ...
 *
 * Split off from Invitations & Stationery Studio's own "business
 * stationery / note card" and "change of address card" item types —
 * same field shape, same prompt structure, just its own tab so
 * stationery and card/invitation shopping don't read as one bucket.
 */
(function () {
  "use strict";
  ...
  var ITEM_TYPE_OPTIONS = ["business stationery / note card", "change of address card"];

  var WORDING_TONE_OPTIONS = ProductHaus.util.sortAlpha([
  ...
  var PRESETS = [
    {
      name: "Modern Business Note Card",
      description: "Clean modern tone, minimal geometric design.",
      apply: { itemType: "business stationery / note card", occasion: "professional thank-you note card", wordingTone: "modern / minimal", visualStyle: "clean geometric line art with a subtle monogram", layout: "flat card" },
    },
    {
      name: "Elegant Change of Address Card",
      description: "Heartfelt tone, elegant script accents.",
      apply: { itemType: "change of address card", occasion: "new home announcement", wordingTone: "heartfelt / sentimental", visualStyle: "elegant script accents with a simple house illustration", layout: "flat card" },
    },
  ];

  function buildInitialState() {
    return {
      itemType: makeField(ITEM_TYPE_OPTIONS[0], ITEM_TYPE_OPTIONS),
  ...
  function getSelectionsByGroup() {
    ...
    return items.length ? [{ title: "Stationery Studio", items: items }] : [];
  }
  ...
  function renderPanel() {
    ...
    wrap.appendChild(ui.renderFieldGroup("Content Details", [
      { label: "Occasion / Theme", field: state.occasion, placeholder: "e.g. \"new home announcement\"" },
      { label: "Key Details to Include", field: state.keyDetails, placeholder: "names, new address, date, contact info" },
    ...
    wrap.appendChild(ui.renderFieldGroup("Visual Style", [
      { label: "Visual Style (optional)", field: state.visualStyle, placeholder: "e.g. \"clean geometric line art with a subtle monogram\"" },
    ], function (entry, changes) { updateField("visualStyle", changes); ProductHaus.ui.renderApp(); }));

    return wrap;
  }

  ProductHaus.stationery = Object.assign({}, store, {
    randomize: randomize,
    reset: reset,
