 ...
 * write values into directly, so there's only one mechanism here:
 * synthetic descriptor entries, read fresh at assembly time and appended
 * to whatever's being generated, the same way Holiday/Theme/Niche
 * already fold in via Business/Voice DNA. No per-category override
 * toggles either — there's only one category of fields, so a kit is
 * either active or it isn't.
 *
...
(function () {
  ...
  var STORAGE_KEY = "graphicsHausBrandKits";
  ...
  function createKit(name) {
    ...
    var kit = { id: "ghbk-" + Date.now() + "-" + Math.floor(Math.random() * 10000), name: (name || "").trim() || "Untitled Brand Kit", createdAt: Date.now(), fields: buildKitFields() };
  ...
  function resolved(field) {
    return GraphicsHaus.engine.resolveFieldValue(field);
  }
  ...
  function renderKitFields(kit) {
    ...
    fieldsWrap.appendChild(ui.renderColorPickerList({
      ...
      onUpdate: function (i, hex) { var next = kit.fields.colors.slice(); next[i] = hex; updateKitColors(kit.id, next); GraphicsHaus.ui.renderApp(); },
      onAdd: function () { updateKitColors(kit.id, kit.fields.colors.concat(["#6B6860"])); GraphicsHaus.ui.renderApp(); },
      onRemove: function (i) { updateKitColors(kit.id, kit.fields.colors.filter(function (_, idx) { return idx !== i; })); GraphicsHaus.ui.renderApp(); },
    }));

    var typographyFields = ui.el("div", { class: "gh-field-group__fields" }, [
      ui.renderFontPreviewField({ label: "Heading Font", field: kit.fields.headingFont }, function (changes) { updateKitField(kit.id, "headingFont", changes); GraphicsHaus.ui.renderApp(); }),
      ui.renderFontPreviewField({ label: "Body Font", field: kit.fields.bodyFont }, function (changes) { updateKitField(kit.id, "bodyFont", changes); GraphicsHaus.ui.renderApp(); }),
    ]);
    ...
    fieldsWrap.appendChild(ui.renderTextSlotList({
      ...
      onAdd: function () { updateKitValues(kit.id, kit.fields.coreValues.concat([""])); GraphicsHaus.ui.renderApp(); },
      onRemove: function (i) { updateKitValues(kit.id, kit.fields.coreValues.filter(function (_, idx) { return idx !== i; })); GraphicsHaus.ui.renderApp(); },
    }));

    fieldsWrap.appendChild(ui.renderFreeTextField({ label: "What the Brand is NOT (optional)", field: kit.fields.notThis, placeholder: "e.g. \"corporate, cold, overly polished\"" }, function (changes) { updateKitField(kit.id, "notThis", changes); GraphicsHaus.ui.renderApp(); }));
  ...
  function renderKitCard(kit, isActive) {
    ...
    if (!isSynced && renamingKitId === kit.id) {
      var titleInput = ui.el("input", { type: "text", class: "gh-saved__item-title-input", value: kit.name });
      var confirmRename = function () { renameKit(kit.id, titleInput.value.trim() || kit.name); renamingKitId = null; GraphicsHaus.ui.renderApp(); };
      titleInput.addEventListener("keydown", function (e) { if (e.key === "Enter") confirmRename(); if (e.key === "Escape") { renamingKitId = null; GraphicsHaus.ui.renderApp(); } });
      titleInput.addEventListener("blur", confirmRename);
      titleRow = ui.el("div", { class: "gh-saved__item-title-row" }, [titleInput]);
    } else {
      var titleChildren = [ui.el("p", { class: "gh-saved__item-title", text: kit.name + (isActive ? " (active)" : "") })];
      if (!isSynced) {
        var renameBtn = ui.el("button", { type: "button", class: "gh-saved__rename-btn", title: "Rename" }, [ui.icon("edit")]);
        renameBtn.addEventListener("click", function () { renamingKitId = kit.id; GraphicsHaus.ui.renderApp(); });
        titleChildren.push(renameBtn);
      }
      titleRow = ui.el("div", { class: "gh-saved__item-title-row" }, titleChildren);
    }
    ...
    activeBtn.addEventListener("click", function () { setActiveKit(isActive ? null : kit.id); GraphicsHaus.ui.renderApp(); });

    var actionBtns = [activeBtn];
    if (isSynced) {
      actionBtns.push(ui.el("span", { class: "gh-brandkit__synced-badge", text: "From your Brand DNA Blueprint™" }));
    } else {
      var expandBtn = ui.el("button", { type: "button", class: "gh-btn gh-btn--small", text: isExpanded ? "Hide Fields" : "Edit Fields" });
      expandBtn.addEventListener("click", function () { expandedKitId = isExpanded ? null : kit.id; GraphicsHaus.ui.renderApp(); });
      var deleteBtn = ui.el("button", { type: "button", class: "gh-btn gh-btn--delete gh-btn--small", text: "Delete" });
      deleteBtn.addEventListener("click", function () { deleteKit(kit.id); GraphicsHaus.ui.renderApp(); });
      actionBtns.push(expandBtn, deleteBtn);
    }

    var children = [titleRow, ui.el("div", { class: "gh-saved__item-actions" }, actionBtns)];
  ...
  function renderSection(root) {
    ...
    if (!kits.length) {
      list.appendChild(ui.el("p", { class: "gh-saved__empty", text: "No Brand Kits yet — create one below to give every studio brand context automatically." }));
    } else {
    ...
    if (!isFull()) {
      var nameInput = ui.el("input", { type: "text", class: "gh-field__custom", placeholder: "New kit name, e.g. \"Client: Wildroot\"" });
      var createBtn = ui.el("button", { type: "button", class: "gh-btn gh-btn--add gh-btn--small", text: "+ Create Brand Kit" });
      ...
      createRow = ui.el("p", { class: "gh-field-group__subtitle", text: "You have " + MAX_OWN_KITS + "/" + MAX_OWN_KITS + " of your own Brand Kits — delete one to create another." });
    }

    var headerChildren = [ui.el("h3", { class: "gh-saved__title" }, [ui.icon("palette"), ui.el("span", { text: "Brand Kit (" + kits.length + "/" + MAX_KITS + ")" })])];
    if (kits.length > 1) {
      ...
      toggleBtn.addEventListener("click", function () { brandKitExpanded = !brandKitExpanded; GraphicsHaus.ui.renderApp(); });
      headerChildren.push(toggleBtn);
    }
    root.appendChild(ui.el("div", { class: "gh-saved" }, [
      ui.el("div", { class: "gh-faq__header" }, headerChildren),
      ui.el("p", { class: "gh-field-group__subtitle", text: "Set colors, fonts, mood, voice, and values once — the active kit automatically informs every studio's output. Up to " + MAX_OWN_KITS + " of your own here, plus up to " + MAX_SYNCED_KITS + " synced in from your Brand DNA Blueprint™ if you have Brand Haus too." }),
      list,
      createRow,
    ]));
  }

  GraphicsHaus.brandKit = {
    MAX_KITS: MAX_KITS,
