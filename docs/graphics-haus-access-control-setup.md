# Access Control Setup — The AI Creator's Graphics Haus

Code side is done: the tool page checks `customer.tags contains "graphics-haus-access"`
...
- **Title**: e.g. "The AI Creator's Graphics Haus — Access Pass"
...
- **Condition**: "Order line items" — Product is [the access product from
  ...
  different product (including any sibling Haus's own access pass).
- **Action**: "Add customer tag" — tag value: `graphics-haus-access`
...
- **The tool page** (gated): assign template `page.graphics-haus` — this is
  the one customers use after buying access.
- **The marketing/preview page** (public): assign template
  `page.graphics-haus-preview` — this is what non-purchasers land on.
...
On the tool page, click into the "Graphics Haus Builder" section and set:

- **Access tag** — leave as `graphics-haus-access` unless you changed the
  Flow's tag in step 2
- **Access product** — pick the product from step 1
- **Learn more link** — point this at the Graphics Haus Preview page from step 3
- **Locked state heading/body** — edit the copy if you want

On the marketing page, click into the "Graphics Haus Preview" section and set:
...
- **As a real customer**: place a test order for the access product, confirm
  the Flow adds the `graphics-haus-access` tag to that customer (Admin →
